...
Once the `conda-forge` channel has been enabled, `debtcollector` can be installed with `conda`:
...
```
mamba install debtcollector
```

It is possible to list all of the versions of `debtcollector` available on your platform with `conda`:
...
```
mamba search debtcollector --channel conda-forge
```
...
mamba repoquery search debtcollector --channel conda-forge

# List packages depending on `debtcollector`:
mamba repoquery whoneeds debtcollector --channel conda-forge

# List dependencies of `debtcollector`:
mamba repoquery depends debtcollector --channel conda-forge
```
